...
class WPEx
{
    ...
    {
        if ($this->get_option("_wpex_show_sale_nag_newsletter", true)) {
            $user = wp_get_current_user();
            ?>
            <div class="update-nag" style="border-left: 4px solid lightgreen;">
                <b>Thanks for using Title Experiments!</b>
                <p style='font-size: 1.15em;'>Do you want my <b>step-by-step guide</b> that can teach you how to use
                experiments to improve your website? <em>It's free!</em></p>
                <p>
                    <form id="wpexNewsletterFrm" style="display: none;" method="POST" action="https://www.getdrip.com/forms/29465887/submissions" target="_blank">
                        <label>Please send it to</label>
                        <input type="text" name="fields[email]" value="<?php echo $user->user_email; ?>" placeholder="your email address" />
                        <input type="submit" class="button button-primary" value="Get it now!">
                    </form>
                    <div>
                        <input id="wpexNewsletterYesBtn" type="button" class="button button-primary" value="Yes, please!">
                        <input id="wpexNewsletterNoBtn" type="button" class="button" value="No, thanks. My website is fine." style='margin-left: 0.5em;' data-nag-id="newsletter">
                    </div>
                </p>
            </div>
            <?php
    ...
    private function _titles($title, $id = null, $ajax = false, $viewed = false)
    {
        ...
        if (!$ajax && $this->get_option("wpex_use_js", false)) {
            ...
            return "<span data-wpex-title-id='$id' style='min-height: 1em; display: inline-block;' data-original='".base64_encode($title)."' $extra_attrs></span>";
        }
    ...
    public function enqueue()
    {
        ...
        wp_register_script('wpextitles', plugins_url('/js/titles.js', __FILE__), array("jquery"), "8.4");
    ...
    {
        wp_enqueue_style('wpexcss', plugins_url('css/wpex.css', __FILE__), array(), "8.4");
        wp_enqueue_script('wpexjs', plugins_url('js/wpex.js', __FILE__), array('jquery'), "8.4");
